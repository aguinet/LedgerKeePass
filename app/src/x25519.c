...
static bool X25519(uint8_t* out, uint8_t const* pub_point, uint8_t const* priv_scalar)
{
  ...
  if (cx_ecfp_scalar_mult(CX_CURVE_Curve25519, outtmp, sizeof(outtmp),
    scalar_, sizeof(scalar_)) == 0) {
    ret = false;
    goto end;
	}
  reverse_bytes(&outtmp[1], &outtmp[X25519_PTSIZE+1]);
