...
# Build speculos
RUN cd /opt/ && git clone --depth=1 --branch=feature/curve25519 https://github.com/aguinet/speculos && \
  rm -rf speculos/.git && \
...
# Install packages
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install --no-install-recommends -y \
  build-essential g++ cmake clang-10 gcc-multilib \
  qemu-user-static \
...
ENV CLANGPATH=/usr/lib/llvm-10/bin/
